...
#ifdef DEBUG
static void print_edgelist(edgelist *edgelist) {
    for (int i = 0; i < edgelist->nedges; i++) {
        printf("edge: address: %p, origin: (%f, %f), twin: %p\n", (void *)edgelist->edges[i], (double)edgelist->edges[i]->origin.x, (double)edgelist->edges[i]->origin.y, (void *)edgelist->edges[i]->twin);
    }
}

static void print_edge(halfedge *e) {
    printf("new edges: edge: %p, twin: %p\n", (void *)e, (void *)e->twin);
}
#endif

static void fill_queue(heap *heap, point *sites, int32_t nsites) {
...
static bp *new_bp(halfedge *edge, point lparabola, point rparabola) {
...
static arc *new_arc(point site, hnode *event) {
...
static void check_new_circle(bintree *tree, heap *heap, bnode *lnode, bnode *mnode, bnode *rnode) {
    bp *b1 = bsuccessor(tree, mnode)->attr; // TODO TODO predecesoor
    bp *b2 = bsuccessor(tree, mnode)->attr;
    if (converge(b1, b2)) {
        circleBottom(((arc *)lnode->attr)->site, ((arc *)mnode->attr)->site, ((arc *)rnode->attr)->site);
        ((arc *)mnode->attr)->circle_event = heap->arr[1]; // wrong
    }
}

static void new_edge(edgelist *edgelist, halfedge **h1, halfedge **h2) {
...
static void handle_site_event(event *event, bintree *tree, heap *heap, edgelist *edgelist) {
    ...
    bnode *old_node = bfindarc(tree, event->site_event.site); // TODO
    ...
    if (old_arc->circle_event) {
        union event *old_event = hremove(heap, old_arc->circle_event); // remove false alarm 
        free(old_event);
    }
    free(old_arc);
    ...
    check_new_circle(tree, heap, bprevleaf(tree, larc_node), larc_node, marc_node);
    check_new_circle(tree, heap, marc_node, rarc_node, bnextleaf(tree, rarc_node));
}

// static void handle_circle_event(event *event, bintree *tree, heap *heap, edgelist *edgelist) {
    // return;
// }
...
static void compute_bounding_box(bintree *tree, edgelist *edgelist) {
    ...
    for (int i = 0; i < edgelist->nedges; i++) {
        ...
        if (edge->origin.x < 0) {
            intersect_lines(&edge->origin, 0, 0, 0, 1, edge->origin.x, edge->origin.y, edge->twin->origin.x, edge->twin->origin.y);
        } 
        if (edge->origin.y < 0) {
            intersect_lines(&edge->origin, 0, 0, 1, 0, edge->origin.x, edge->origin.y, edge->twin->origin.x, edge->twin->origin.y);
        }
        if (edge->origin.x > 1) {
            intersect_lines(&edge->origin, 1, 0, 1, 1, edge->origin.x, edge->origin.y, edge->twin->origin.x, edge->twin->origin.y);
        }
        if (edge->origin.y > 1) {
            intersect_lines(&edge->origin, 0, 1, 1, 1, edge->origin.x, edge->origin.y, edge->twin->origin.x, edge->twin->origin.y);
        }
    }
}

void fortunes(point *sites, int32_t nsites, edgelist *edgelist) {
    heap *heap = init_heap();
